...
export const Libraries = () => {
    ...
    const fetchLibraries = async () => {
        setIsLoading(true);
        try {
            const response = await api.get('/libraries'); // removed generic type to rely on inferred or explicit types if needed, but keeping simple for now
            ...
            {isLoading ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 xl:grid-cols-5 gap-4">
                    {libraries.map((library) => (
                        <div key={library.id} className="bg-white p-6 rounded-lg shadow-sm border border-gray-100 flex flex-col justify-between group hover:shadow-md transition-shadow">
                            <div>
                                <h3 className="font-semibold text-lg text-gray-900">{library.name}</h3>
                                {library.description && (
                                    <p className="text-sm text-gray-500 mt-2">{library.description}</p>
                                )}
                            </div>
                            <div className="flex justify-end mt-4 pt-4 border-t border-gray-50 opacity-0 group-hover:opacity-100 transition-opacity">
